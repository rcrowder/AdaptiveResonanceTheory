...
if (init_vals.datasrc==1)
    dataSetNum=4;
elseif (init_vals.datasrc==2)
    dataSetNum=5;
elseif (init_vals.datasrc==3)
    dataSetNum=2;
elseif (init_vals.datasrc==4)
    dataSetNum=3;
elseif (init_vals.datasrc==5)
    dataSetNum=6;
elseif (init_vals.datasrc==6)
    dataSetNum=7;
elseif (init_vals.datasrc==7)
    dataSetNum=9;
elseif (init_vals.datasrc==8)
    dataSetNum=10;
elseif (init_vals.datasrc==9)
    dataSetNum=11;
elseif (init_vals.datasrc==10)
    dataSetNum=12;
elseif (init_vals.datasrc==11)
    dataSetNum=13;
end
...
try
    dataStruct_fuzzyART(dataSetNum).test_output=dataStruct_fuzzyART(dataSetNum).test_output(:);
catch
...
%inputNet.alpha=10^(-8);
inputNet.alpha=init_vals.alpha;

%inputNet.beta=1;
inputNet.beta=init_vals.beta;


%inputNet.gamma=0.0000001;
inputNet.gamma=init_vals.gamma;

%inputNet.rho_bar=0;
inputNet.rho_bar=init_vals.rho;


%inputNet.plotSteps=0;
inputNet.plotSteps=init_vals.plot_steps;

inputNet.model_num=32;

%Train for number of epochs
%inputNet.numEpochs=1;
inputNet.numEpochs=init_vals.epochs;
...
if ismember(dataSetNum,[1 2 3 4 5 6 7])
    ...
    if (perc_error>0)
        orig_dataset_labels=dataStruct_fuzzyART(dataSetNum).training_output;
        dataStruct_fuzzyART(dataSetNum).training_output=addClassError(dataStruct_fuzzyART(dataSetNum).training_output,inputNet.perc_error);
        temp_labels_noise=dataStruct_fuzzyART(dataSetNum).training_output;
    elseif (var_error_features>0)
        orig_dataset_features=dataStruct_fuzzyART(dataSetNum).training_input;
        dataStruct_fuzzyART(dataSetNum).training_input=max(min(dataStruct_fuzzyART(dataSetNum).training_input+randn(size(dataStruct_fuzzyART(dataSetNum).training_input))*var_error_features,1),0);
        temp_features_noise=dataStruct_fuzzyART(dataSetNum).training_input;
    end
    if ~exist('dataTestUnif','var')
        load ARTMAP_data_test dataTestUnif dataTestOutUnif test_x test_y
    end
    dataStruct_fuzzyART(dataSetNum).test_input=dataTestUnif;
    if dataSetNum>1
        dataStruct_fuzzyART(dataSetNum).test_output=dataTestOutUnif{floor(dataSetNum/2)}';
    end
end
...
[a,b,c]=Default_ARTMAP(dataStruct_fuzzyART(dataSetNum),inputNet);
if ismember(dataSetNum,[1 2 3 4 5 6 7])
    if (perc_error>0)
        inputNet.lambda_Attention=0;
        [a_zero,b_zero,c_zero]=Default_ARTMAP(dataStruct_fuzzyART(dataSetNum),inputNet);
        disp('Same training order lambda = 0')
        disp(sum(a_zero==dataStruct_fuzzyART(dataSetNum).test_output)*100/length(a_zero));
        labels_switch_positions=find(orig_dataset_labels~=temp_labels_noise);
        disp(intersect(labels_switch_positions,c.NI_fail))
        dataStruct_fuzzyART(dataSetNum).training_output=orig_dataset_labels;
    elseif (var_error_features>0)
        dataStruct_fuzzyART(dataSetNum).training_input=orig_dataset_features;
    end
end
...
if (c.M==2) && (inputNet.plotSteps==0)
    ...
    if dataSetNum~=1
        ...
        imagesc(test_x(:),test_y(:),(reshape(a+(a~=dataStruct_fuzzyART(dataSetNum).test_output).*((a==2)-(a==1)),length(test_x),length(test_x))));
        set(gca,'ydir','normal')
        hold on
    ...
    axis tight
    axis([-.1 1.1 -.1 1.1]);
    %     title(['fuzzy ARTMAP: feature selection in ART search cycle; Type (One Epoch)'],'fontsize',12);
    %      ylabel(['\lambda = ' num2str(c.lambda_Attention,3)],'fontsize',12)
    if dataSetNum~=1
        xlabel(['Accuracy : ', num2str(sum(dataStruct_fuzzyART(dataSetNum).test_output==a)*100/length(dataStruct_fuzzyART(dataSetNum).test_output),3),'%, Committed Nodes: ',num2str(c.C), ......
            ],'fontsize',12);
    end
    ...
    if (perc_error>0)
        ...
        if dataSetNum~=1
            ...
            a_test=dataStruct_fuzzyART(dataSetNum).test_output;
            colormap([1 .8 .8;.8 .8 1])
            imagesc(test_x(:),test_y(:),(reshape(a_test+(a_test~=dataStruct_fuzzyART(dataSetNum).test_output).*((a_test==2)-(a_test==1)),length(test_x),length(test_x))));
            set(gca,'ydir','normal')
            hold on
        ...
        axis tight
        axis([-.1 1.1 -.1 1.1]);
        %         title(['fuzzy ARTMAP: feature selection in ART search cycle; Type ' num2str(c_zero.model_num) ' (One Epoch)'],'fontsize',12);
        %         ylabel(['\lambda = ' num2str(c_zero.lambda_Attention,3)],'fontsize',12)
        %         if dataSetNum~=1
        %             xlabel(['Accuracy% : ', num2str(sum(dataStruct_fuzzyART(dataSetNum).test_output==a_zero)*100/length(dataStruct_fuzzyART(dataSetNum).test_output),3),', C Nodes: ',num2str(c_zero.C), ...
        %                 ', \alpha: ',num2str(c_zero.alpha)......
        %                 ', Training Pts : ',num2str(length(dataStruct_fuzzyART(dataSetNum).training_input))...
...
elseif ismember(c.M,6)
    ...
    [a_nodep,b_nodep,c_nodep]=Default_ARTMAP(dataStruct_fuzzyART(dataSetNum),inputNet);
    test_out=num2str(dataStruct_fuzzyART(dataSetNum).test_input');
...
elseif c.M==41
    ...
    [conf_nonnorm,conf_norm,avg_acc]= confMatrixPlot(dataStruct_fuzzyART(dataSetNum).test_output(:),a(:),c,dataStruct_fuzzyART(dataSetNum).labels,title_boston);


elseif strcmp(dataStruct_fuzzyART(dataSetNum).description,'movie_genres')
    b_mean=mean(b,1);
    b_std=std(b,1);
    b_new=(b-repmat(b_mean,length(dataStruct_fuzzyART(dataSetNum).test_output),1))./repmat(b_std,length(dataStruct_fuzzyART(dataSetNum).test_output),1);
    ...
    sigma_new=(sigma-repmat(sigma_mean,1,length(dataStruct_fuzzyART(dataSetNum).test_output)))./repmat(sigma_std,1,length(dataStruct_fuzzyART(dataSetNum).test_output));
    [dum,classes]=max(sigma,[],1);

    [conf_nonnorm,conf_norm,avg_acc]= confMatrixPlot(dataStruct_fuzzyART(dataSetNum).test_output(:),classes,c,dataStruct_fuzzyART(dataSetNum).labels,'');
end

disp('Output class predictions are stored in variable ''a''.');
disp('Distributed output predictions are stored in ''b''.');
disp('The ARTMAP network details are stored in ''c''');
