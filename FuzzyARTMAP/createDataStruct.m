...
%Usage: createDataStruct(in_mat,row_col_string,labels_end_begin,perc_train,data_description,varargin)
%
%Examples:
% 1) The dataset raw_data contains 22000 rows (records) and 43 columns
%(The first column contains class labels and the subsequent 42 columns are features)
% To create a training set with 70% of the records:
% dataStructTemp=createDataStruct(raw_data,'rows','begin','my_dataset_name')

% 2) The dataset raw_data contains 22000 rows (records) and 43 columns
%(42 features and the 43rd row contains class labels)
% To create a training set with 70% of the records:
% dataStructTemp=createDataStruct(raw_data,'rows','end','my_dataset_name')

% 3) The dataset raw_data contains  22000 rows (records) and 42 columns
%(features) and a separate column vector class_val.
% To create a training set with 70% of the records:
% dataStructTemp=createDataStruct([raw_data class_val],'rows','end','my_dataset_name')
%
% 4) Consider a dataset raw_data with 42 rows (features) and 22000 columns
%(records) and a column vector class_val.
% To create a training set with 70% of the records:
% dataStructTemp=createDataStruct([raw_data class_val],'cols','end','my_dataset_name')
%
%Note: This code automatically determines feature minima and maxima
%from the provided dataset and rescales the features into the [0,1] range
%
